...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn("border-b border-border last:border-0 hover:bg-hms-muted/50", className)}
      {...props}
    />
  ),
);
...
>(({ className, ...props }, ref) => (
  <th
    ref={ref}
    className={cn(
      "px-3 py-2 text-left font-mono text-[10.5px] font-medium uppercase tracking-wider text-fg-muted",
      className,
    )}
    {...props}
  />
